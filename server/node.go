...
func (vm *AutoScalerServerNode) launchVM(c types.ClientGenerator, nodeLabels, systemLabels types.KubernetesLabel) error {
	...
	} else if vm.VMUUID, err = config.Create(vm.NodeName, userInfo.GetUserName(), userInfo.GetAuthKeys(), vm.serverConfig.CloudInit, network, vm.Memory, vm.CPU, vm.Disk, vm.NodeIndex); err != nil {
...
func (vm *AutoScalerServerNode) setProviderID(c types.ClientGenerator) error {
	if vm.serverConfig.Distribution == nil || *vm.serverConfig.Distribution == types.KubeAdmDistributionName {
...
func (vm *AutoScalerServerNode) generateProviderID() string {
	if vm.serverConfig.Distribution != nil {
		switch *vm.serverConfig.Distribution {
